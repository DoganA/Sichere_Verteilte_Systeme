...
def send(user, recipient, message_sent):
    ...
    if not key:
        ...
        # generate public key
        A = diffiehellmann.get_public_key(int(getStringFromText(private_key_location))) 
    ...
    #write_string_to_file(recipient_short + "/mail_" + user + ""+ timestamp +"" ".txt", base64_ecnoded_msg) 
    write_string_to_file(recipient_short + "/mail_" + user + ".txt", base64_ecnoded_msg) 
...
def receive(user):
    ...
    for mail in prefixed:
        # read mail content
        text = getStringFromText(short_from_mail(user) + "/" + mail)
        print(mail)
        print("read: " + str(text))
        # sender account
...
def save_dh(my_account, other_account, key):
    ...
    write_string_to_file(location, key)
    return True
...
def read_dh(my_account, other_account):
    location = short_from_mail(my_account) + "/" + short_from_mail(other_account) + "_public.txt"
    if os.path.isfile(location):
        return int(getStringFromText(location))
...
def diffie_hellmann_key(account1, account2):
    ...
    if not os.path.isfile(private_key_location):
        ...
        write_string_to_file(private_key_location, str(private_key))
    ...
    if not B:
        ...
        return int(getStringFromText(k_location))
    else:
        a = int(getStringFromText(private_key_location))
        B = int(B)
...
def write_dh_k(a, B, location):
    ...
    write_string_to_file(location, str(K))
    return K
